...
def cash_flow(
    ...
    elect_df = pd.DataFrame(
        {
            ...
            "electricity_price": [electricity_price] * len(range(1983, 2100)),
        }
    ).set_index("year")
    CPI_inflator = (
    ...
    specified_replacement_cost = [
        0
        if i <= 1
        else -(0.15 * direct_cap / (CPI_inflator)) * CPI_inflator
    ...
    depr_charge = [
        -depr_cash_flow[i] if i < 40 else -sum(depr_cash_flow[i:])
    ...
    lcoh_O2_revenue = (
        (-(lcoh_oxygen_revenue) * (1 - total_tax_rate))
        / lcoh_H2_sales
        * (1 + var.inflation_rate) ** 1
        / inflation_factor
    )
...
def create_O2revenue_plot(
    ...
    lcoh_list = []
    for i in O2_value:
        lcoh, lcoh_O2 = cash_flow(
            cap_factor=capacity_factor,
