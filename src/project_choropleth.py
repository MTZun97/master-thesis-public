...
def generate_choropleth(status="all"):
    ...
    country_names = df.groupby(["country"]).sum(numeric_only=True).reset_index()

    country_names["country_count"] = country_names["country"].apply(get_country_name)
    value = "normalized_capacity_Mwel"

    hover_template = (
        "<b>%{customdata[0]}</b><br>" + "Capacity: %{customdata[1]:.2f} MWel/y"
    )
    ...
    fig.update_coloraxes(
        colorbar=dict(
            title=dict(
                ...
                + "MWel/y"
                + "</b>",
                font=dict(family="Arial Bold", size=24, color="black"),
            ),
