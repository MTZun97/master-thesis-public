...
file_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "data", "lcoh_data.xlsx")
)
...
def get_earliest_year(df_data, target):
    ...
    for country, df_country in df_data.items():
        earliest_years = {
            source: df_country[source].loc[df_country[source] < target].index[0]
            if df_country[source].loc[df_country[source] < target].index.any()
            else None
...
def create_trace(df_low, df_high, color_low, color_high, sources_symbols, countries):
    ...
    for source, symbol in sources_symbols.items():
        for country in countries:
            country_data_low = df_low[
                (df_low["Country"] == country) & (df_low["Source"] == source)
            ]
            country_data_high = df_high[
                (df_high["Country"] == country) & (df_high["Source"] == source)
            ]
            ...
            if not country_data_low.empty:
                traces.append(
                    go.Scatter(
                        ...
                        y=[
                            country for _ in range(len(country_data_low["Year"].values))
                        ],
                        mode="markers",
            ...
            if not country_data_high.empty:
                traces.append(
                    go.Scatter(
                        ...
                        y=[
                            country
                            for _ in range(len(country_data_high["Year"].values))
                        ],
                        mode="markers",
            ...
            if (
                country in ["Canada", "Sweden", "Denmark"] and source == "solar_lcoe"
            ) or (
                country in ["Australia", "Netherlands"]
                and source == "onshore_wind_lcoe"
            ):
                traces.append(
                    go.Scatter(
                        x=[2047],
...
def create_timeline_plot(cost_target):
    ...
    results_df_low = results_df_low.melt(
        id_vars=["Country"], var_name="Source", value_name="Year"
    )

    results_df_high = get_earliest_year(df_high, cost_target)
    results_df_high = results_df_high.melt(
        id_vars=["Country"], var_name="Source", value_name="Year"
    )
    ...
    fig = make_subplots(
        rows=1,
        cols=2,
        subplot_titles=("Solar Energy", "Onshore Wind Energy"),
        shared_yaxes=True,
        horizontal_spacing=0.05,
        vertical_spacing=0.1,
    )
    for trace in create_trace(
        results_df_low,
        results_df_high,
        colors["Optimistic"],
        colors["Pessimistic"],
