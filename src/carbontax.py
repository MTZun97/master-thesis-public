...
def create_carbontax_plot(
    ...
    capacity_factor_CO2,
    electricity_price_CO2=[0, 100],
):
    electricity_price_CO2 = np.arange(
        electricity_price_CO2[0], electricity_price_CO2[1], 5
    ).round(3)
    ...
    lcoh_smr_ccus_tax = [
        i + ((carbon_tax / 100) * (1 - capture_rate / 100)) for i in lcoh_smr_ccus
    ]

    lcoh_list = []
    for i in electricity_price_CO2:
        lcoh, _ = cash_flow(
            cap_factor=capacity_factor_CO2,
            electrolyzer_cost=electrolyzer_cost_CO2,
            electricity_price=i / 1000,
            O2=0,
            electrolyzer_efficiency=electrolyzer_efficiency_CO2,
            mech_percent=0.3,
            elect_percent=0.2,
    ...
    data_dict = {
        "electricity_price": electricity_price_CO2,
        "LCOH": lcoh_list,
        "LCOH - SMR": lcoh_smr,
        "LCOH - SMR + CCUS": lcoh_smr_ccus,
        "LCOH - SMR + Carbon Tax": lcoh_smr_tax,
