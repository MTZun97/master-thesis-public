...
settings = {
    'optimistic': {'alk': {'params': {"current_density": 0.5, "stack_percent": 0.5, "mech_percent": 0.2, "elect_percent": 0.3}, 'cost': "alk_doublef_sl", 'eff': "eff_singlef_alk"}, 
    ...
    'pessimistic': {'alk': {'params': {"current_density": 0.5, "stack_percent": 0.5, "mech_percent": 0.2, "elect_percent": 0.3}, 'cost': "alk_doublef", 'eff': "eff_singlef_alk"}, 
...
countries = ["Australia", "Germany", "China", "Netherlands", "Japan", "Canada", "United States", "United Kingdom",
             "Sweden", "Spain", "France", "Denmark", "Italy"]

df_low = {}
for country in countries:
    data = []
    setting = settings["optimistic"]["pem"]
...
for country in countries:
    data = []
    setting = settings["pessimistic"]["pem"]
    for index in df.index:
        electrolyzer_cost = df.loc[index, setting['cost']]
        electrolyzer_efficiency = df.loc[index, setting['eff']]
        lcoh_row = {"Index": index}
...
lcoh_country = {"optimistic": results_df_low, "pessimistic": results_df_high}
with pd.ExcelWriter('excel/lcoh_country.xlsx') as writer:
    for key in lcoh_country:
        lcoh_country[key].to_excel(writer, sheet_name=key)
...
fig.update_layout(
    ...
    xaxis=dict(title='<b>Year<b>', range=[2010, 2050], tickfont=dict(size = 12)),
    yaxis=dict(title='<b>Country<b>', tickfont=dict(size = 12)),
    autosize=False,
    width=1000,
    height=700,
)
